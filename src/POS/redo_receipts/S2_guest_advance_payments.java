...
import POS.advance_payment.S1_guest_advance_payment;
 ...
 * @author Maytopacka
 */
public class S2_guest_advance_payments {
    ...
    public static List<to_guest_advance_payment> ret_data(String date_from, String date_to, String guests) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                to_guest_advance_payment to = new to_guest_advance_payment(id, room_id, guest_name, guest_id, date_added, amount_paid, status, user_name, usd, room_guest_id, peso_on_bank, usd_on_bank, is_peso_on_bank, is_usd_on_bank, bank_no, bank, card_type, card_holder, card_no, approval_code, credit_amount, credit_addtl, credit_rate);
                datas.add(to);
            }
            return datas;
        } catch (Exception e) {
    ...
    public static String increment_id(String id) {
//        String id = "0";
        try {
            Connection conn = MyConnection1.connect();
            String s0 = "select table_name from " + MyDB.getNames() + ".tables where id='"+id+"'";
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery(s0);
            if (rs.next()) {
                id = rs.getString(1);
